...
| Skill | Role |
|---|---|
| [adr-discovery](skills/adr-discovery/SKILL.md) | Gathers project context before drafting, zero-hallucination Q&A, writes `docs/architecture/discovery-brief.md`. Optional live LikeC4 diagram mode. |
| [draft-adr](skills/draft-adr/SKILL.md) | Co-drafts an ADR through 7 phases (Understand → Context → Options → Decide → Draft → Self-Critique → Save). |
| [adr-critique](skills/adr-critique/SKILL.md) | Audits a pre-existing/legacy ADR line-by-line against the checklist. |
| [c4-model](skills/c4-model/SKILL.md) | Generates a canonical-C4 LikeC4 model (Context + Container views only). |
...
- **Shared term definitions** (Component = C4 Container, System, Architectural characteristic, Tension, RFC, Fitness function) are restated in each skill so the skill is self-contained. Keep the definitions aligned across files.
